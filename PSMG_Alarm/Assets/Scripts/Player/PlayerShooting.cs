...
public class PlayerShooting : MonoBehaviour
{
    ...
    private void FireBullet(Vector3 aimPosition)
    {
        ...
        if (weaponTyp == weaponTyps.mg && mgUpgrade > 0)
        {
            ...
            if (mgUpgrade == 2)
            {
                GameObject bulletInstance1 = InGameNetworking.NetworkInstantiate(weapon[(int)weaponTyp].weapon, transform.position, transform.rotation * Quaternion.Euler(0, 0, 3f), true);
                GameObject bulletInstance2 = InGameNetworking.NetworkInstantiate(weapon[(int)weaponTyp].weapon, transform.position, transform.rotation * Quaternion.Euler(0, 0, -3f), true);
